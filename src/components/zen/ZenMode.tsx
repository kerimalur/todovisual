...
export function ZenMode() {
  ...
  const [phase, setPhase] = useState<Exclude<ZenPhase, 'focus'>>('select-task');
  ...
  const runningTask = useMemo(() => {
    if (!timer.currentTaskId) return null;
    return tasks.find((t) => t.id === timer.currentTaskId) ?? null;
  }, [timer.currentTaskId, tasks]);

  const currentPhase: ZenPhase = timer.isRunning ? 'focus' : phase;
  const activeTask = selectedTask ?? runningTask;
  ...
  // Document title
  useEffect(() => {
    if (currentPhase === 'focus' && timer.isRunning) {
  ...
  }, [currentPhase, timer.secondsRemaining, timer.isRunning]);
  ...
  const handleStartTimer = () => {
    if (activeTask) {
      startTimer(timerMinutes, activeTask.id);
  ...
  useEffect(() => {
    if (currentPhase === 'focus' && timer.isRunning && !timer.isPaused) {
  ...
  }, [currentPhase, timer.isRunning, timer.isPaused]);
    ...
    <div className="fixed inset-0 bg-[#191919] flex flex-col z-[100]">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-[#2f2f2f]">
        ...
        <div className="flex items-center gap-2">
          {currentPhase === 'focus' && timer.isRunning && !timer.isPaused && (
          ...
          <span className="text-[#9b9b9b] text-xs tracking-wider uppercase">
            {currentPhase === 'select-task' && 'Aufgabe wählen'}
            {currentPhase === 'set-timer' && 'Timer einstellen'}
            {currentPhase === 'focus' && 'Fokus Mode'}
          </span>
        </div>
        <button 
          onClick={handleExit} 
          ...
          title={currentPhase === 'focus' && timer.isRunning ? 'Fokus unterbrechen' : 'Schließen'}
        >
          <X size={18} />
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 overflow-auto">
        ...
        {/* Phase 2: Set Timer */}
        {currentPhase === 'set-timer' && activeTask && (
